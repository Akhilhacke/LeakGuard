...
export async function POST(request: Request) {
  try {
    ...
    const headers: Record<string, string> = { 'User-Agent': 'LeakGuard-Scanner' };
      ...
      try {
        ...
        if (treeRes.ok) {
            ...
            if (textFiles.length === 0) {
                return NextResponse.json({ error: 'No scannable text files found in this repository.' }, { status: 400 });
            }
            ...
            else return NextResponse.json({ error: 'Could not fetch file contents from repository.' }, { status: 400 });
        } else {
            const errorData = await treeRes.json().catch(() => ({}));
            return NextResponse.json({ 
                error: `GitHub fetch failed: ${treeRes.statusText}`,
                details: errorData.message || 'Make sure the repository is public.'
            }, { status: 400 });
        }
      } catch (e: any) {
        console.error('GitHub fetch failed', e);
        return NextResponse.json({ error: `GitHub connection error: ${e.message}` }, { status: 500 });
      }
    }
    ...
    lines.forEach((line: string, index: number) => {
      ...
      PATTERNS.forEach(patternObj => {
        ...
        while ((match = patternObj.regex.exec(line)) !== null) {
          ...
          // Skip obvious test/example placeholder passwords only
